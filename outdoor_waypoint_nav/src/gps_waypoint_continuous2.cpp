...
int main(int argc, char** argv)
{
    ...
    for(iter = waypointVect.begin(); iter < waypointVect.end() && ros::ok(); iter++)
    {
        ...
        // this means that neither controller 1 or controller 2 are on the last waypoint so continue incrementing iter by 2
        if(iter < (waypointVect.end() - 2))
        {
            iter++;
            latiNext = iter->first;
            ...
            // this means that controller 1 is on the second last point so increment iter once more and shutdown after controller 2 is done
        else if(iter == (waypointVect.end() - 2))
        {
            ...
            // this means that controller 1 is on its last waypoint and controller 2 should wait for controller 1 to shutdown
        else if(iter == (waypointVect.end() - 1))
        {
        ...
        // continue using controller 2, else wait for controller 1 to end
